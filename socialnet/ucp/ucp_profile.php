		...
		switch ($module)
		{
			...
			case 'relations':
				...
				switch ($action)
				{
					...
					case 'delete_relation': // delete relation
						$this->delete_relation();
						break;

					case 'approve_relation': // approve relationship or family relation
					...
					if ($submit)
					{
						...
						if (!sizeof($error))
						{
							if ($data['relationship_user'] != '' && $relationship_with_partner)
							{
								...
								$db->sql_escape($data['relationship_user']) . '"';
								...
								if (!sizeof($error))
								{
									...
									else
									{
										...
										if (isset($socialnet->modules_obj['notify']) && is_object($socialnet->modules_obj['notify']))
										{
											$socialnet->modules_obj['notify']->ntf_check_RELATIONSHIP($new_relationship_id, $new_relationship['user_id']);
										}
									}
								}
							}
						...
						if ($data['family_user'] != '' && $data['family_status'] != '0')
						{
							$sql = 'SELECT user_id, username
                          			FROM ' . USERS_TABLE . '
                            		WHERE username = "' .
							$db->sql_escape($data['family_user']) . '"';
							$result = $db->sql_query($sql);
							...
							if (!sizeof($error))
							{
								if (!$family['user_id'])
								{
									$sql = 'INSERT INTO ' . SN_FAMILY_TABLE . ' (user_id, relative_user_id, status_id, approved, anniversary, family, name)
												VALUES (' .
									$user->data['user_id'] . ', 0, ' . $data['family_status'] . ', 1, "", 1, "' . $data['family_user'] . '")';
									$db->sql_query($sql);
								...
								else
								{
									$sql = 'INSERT INTO ' . SN_FAMILY_TABLE . ' (user_id, relative_user_id, status_id, approved, anniversary, family)
												VALUES (' .
									$user->data['user_id'] . ', ' . $family['user_id'] . ', ' . $data['family_status'] . ', 0, "", 1)';
									$db->sql_query($sql);
									...
									if (isset($socialnet->modules_obj['notify']) && is_object($socialnet->modules_obj['notify']))
									{
										$socialnet->modules_obj['notify']->ntf_check_FAMILY($family_id, $family['user_id'], $data['family_status']);
									}
								}
							}
						}
					...
					$sql = 'SELECT f.id, f.status_id, f.anniversary, f.approved, f.relative_user_id, f.family, f.name, u.username, u.user_colour, u.user_avatar, u.user_avatar_type, u.user_avatar_width, u.user_avatar_height
	...
	function approve_relation()
	{
		...
		if (confirm_box(true))
		{
			...
			if ($row['family'] == 0)
			{
				...
				if ($vice_versa)
				{
					$sql = 'INSERT INTO ' . SN_FAMILY_TABLE . ' (user_id, relative_user_id, status_id, approved, anniversary, family)
                    VALUES (' .
					$user->data['user_id'] . ', ' . $row['user_id'] . ', ' . $row['status_id'] . ', 1, "' . $row['anniversary'] . '", 0)';
					$db->sql_query($sql);
			...
			else
			{
				...
				if ($vice_versa && $family_status_id)
				{
					$sql = 'INSERT INTO ' . SN_FAMILY_TABLE . ' (user_id, relative_user_id, status_id, approved, anniversary, family)
                    VALUES (' .
					$user->data['user_id'] . ', ' . $row['user_id'] . ', ' . $family_status_id . ', 1, "", 1)';
					$db->sql_query($sql);
		...
		else
		{
			...
			else
			{
				...
				//$template->set_filenames(array('sn_up_confirm' =>'socialnet/confirm_relation_body.html'));
				//$content = $template->assign_dispay('sn_up_confirm');
