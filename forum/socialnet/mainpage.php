...
if (!class_exists('socialnet_mainpage'))
{
	...
	class socialnet_mainpage
	{
		...
		function load($mode)
		{
			...
			switch ($mode)
			{
				...
				case 'snMpNewestEntries':
					...
					$a_mp_entries = $this->mp_load_entries($last_entry_id, 15, false);
		 ...
		 * Nacti vstupy na MP
		 *
		 * @param integer $last_entry_id Poslední načtený vstup na MP
		 ...
		 */
		function mp_load_entries($last_entry_id, $entries_limit = 15, $older = true)
		{
			...
			$sql_ary = array(
				...
				'WHERE'		 => $db->sql_in_set('sn_e.user_id', $my_friends, false, true) . (($last_entry_id != 0) ? ' AND sn_e.entry_id ' . ($older ? '<' : '>') . ' ' . $last_entry_id : '') . ' AND ' . $db->sql_in_set('sn_e.entry_type', $avail_entry_types),
			...
			for ($i = 0; $i < $entries_limit && isset($entries_rowset[$i]); $i++)
			{
				...
				switch ($entries_row['entry_type'])
				{
					case SN_TYPE_NEW_STATUS:
						$a_mp_entries[] = $this->mp_load_status_entry($entries_row['entry_id'], $entries_row['entry_type'], $entries_row['user_id'], $entries_row['entry_target'], $entries_row['entry_time']);
						break;
					case SN_TYPE_NEW_FRIENDSHIP:
						$a_mp_entries[] = $this->mp_load_friends_entry($entries_row['entry_id'], $entries_row['entry_type'], $entries_row['user_id'], $entries_row['entry_target']);
						break;
					case SN_TYPE_PROFILE_UPDATED:
						$a_mp_entries[] = $this->mp_load_profile_entry($entries_row['entry_id'], $entries_row['entry_type'], $entries_row['user_id'], $entries_row['entry_target'], $entries_row['entry_additionals']);
						break;
					case SN_TYPE_NEW_FAMILY:
						$a_mp_entries[] = $this->mp_load_family_relation_entry($entries_row['entry_id'], $entries_row['entry_type'], $entries_row['user_id'], $entries_row['entry_target'], $entries_row['entry_additionals']);
						break;
