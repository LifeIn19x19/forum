...
require 'imageSmoothArc_optimized.php';
...
class GoDiagram
{
	...
	if ($this->width < 1 || $this->height < 1)
		...
		//$extend = ceil(floatval(strlen($this->title))/floatval($this->width))*9;
          //      $this->globalOffset = $this->height;
		$this->dim = new GoDiagramDimensions($this->width, $this->height);
		//$this->dim->height = $this->dim->height+($numLines/3);
		//$this->dim->offset_y = $this->dim->offset_y+$numLines
        }
...
public function unhtmlentities($string)
{
    ...
    protected function drawDiagram()
    {
     	   $lines = array();
		   $numLines = 0;
		   $splitComment = wordwrap(html_entity_decode($this->title), floor(($this->dim->width+5)/imagefontwidth(3)), "~*)", false);
		   ...
		   $this->addedOffset = 15*$numLines;
	// create image
	$this->img = ImageCreate($this->dim->width, $this->dim->height+$this->addedOffset);
	...
	if($this->addedOffset != 0) {
		    //ImageFill($this->img, 0, $h1, $this->colors['white']);
		    imagefilledrectangle($this->img, 0, $this->dim->height, $this->dim->width, $this->dim->height+$this->addedOffset, $this->colors['white']);
	}
    ...
    /**
     * Draw a single stone.
     * @param int $centerX X position in pixel
     * @param int $centerY Y position in pixel
     * @param int $Crim Color index of rim
     * @param int $Cinside Color index of stone
     */
    protected function drawStone ($centerX, $centerY, $Crim, $Cinside)
    {
      // $d = $this->dim->radius*2;
       //ImageArc($this->img, $x, $y, $d, $d, 0, 360, $Crim);
       //imageSmoothArc($this->img, $x, $y, $d, $d, $Crim, 0, 360);


	  $maxTransparency = 0x7F; // 127
	  $color = $Cinside;
	  $radiusX2 = $this->dim->radius * $this->dim->radius;
	  $radiusY2 = $this->dim->radius * $this->dim->radius;
	  // upper and lower halves
	  $quarter = round($radiusX2 / sqrt($radiusX2 + $radiusY2));
	  for ($x = 0; $x <= $quarter; $x++)
	  {
	    $y = $this->dim->radius * sqrt(1-$x*$x/$radiusX2);
	    $error = $y - floor($y);
	    $transparency = round($error * $maxTransparency);
	    $alpha  = $color | ($transparency << 24);
	    $alpha2 = $color | (($maxTransparency - $transparency) << 24);
	  
		imagesetpixel($this->img, $centerX + $x, $centerY + floor($y), $alpha);
		imagesetpixel($this->img, $centerX - $x, $centerY + floor($y), $alpha);
		imagesetpixel($this->img, $centerX + $x, $centerY - floor($y), $alpha);
		imagesetpixel($this->img, $centerX - $x, $centerY - floor($y), $alpha);
		
		imagesetpixel($this->img, $centerX + $x, $centerY + floor($y)+1, $alpha2);
		imagesetpixel($this->img, $centerX - $x, $centerY + floor($y)+1, $alpha2);
		imagesetpixel($this->img, $centerX + $x, $centerY - floor($y)+1, $alpha2);
		imagesetpixel($this->img, $centerX - $x, $centerY - floor($y)+1, $alpha2);
	  }
	  ...
	  for ($y = 0; $y <= $quarter; $y++)
	  {
	    $x = $this->dim->radius * sqrt(1-$y*$y/$radiusY2);
	    $error = $x - floor($x);
	    $transparency = round($error * $maxTransparency);
	    $alpha  = $color | ($transparency << 24);
	    $alpha2 = $color | (($maxTransparency - $transparency) << 24);
	    
	    imagesetpixel($this->img, $centerX + floor($x), $centerY + $y, $alpha);
		imagesetpixel($this->img, $centerX - floor($x), $centerY + $y, $alpha);
		imagesetpixel($this->img, $centerX + floor($x), $centerY - $y, $alpha);
		imagesetpixel($this->img, $centerX - floor($x), $centerY - $y, $alpha);
		
		imagesetpixel($this->img, $centerX + floor($x)+1, $centerY + $y, $alpha2);
		imagesetpixel($this->img, $centerX - floor($x)+1, $centerY + $y, $alpha2);
		imagesetpixel($this->img, $centerX + floor($x)+1, $centerY - $y, $alpha2);
		imagesetpixel($this->img, $centerX - floor($x)+1, $centerY - $y, $alpha2);
	  }
