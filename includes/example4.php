...
class GoDiagramDimensions
{
    ...
    var $offset_x;	// origin (0,0) of board itself
    var $offset_y;
...
{
    // misc
    var $content;         // raw copy of diagram contents (single string)
    var $linkmap;         // array of imagemap links (bracketlinks)
    var $specialMarkup;	// special markup inside curly brackets
    var $dim;		// holds DiagramDimensions

    // values extracted from the title line
    var $firstColor;	// 'B' or 'W'
    var $coordinates;	// boolean
    var $boardSize;	// integer (from title line)
    var $title;		// raw text of title
    var $startMoveNum;	// starting move number for moves in diagram

    // goban properties
    var $goban;		// board (string) without borders
    var $width;		// actual width
    var $height;          // actual height
    var $coordx;          // X offset for coordinate numbering
    var $coordy;          // Y offset for coordinate numbering
    var $moves;
    var $markup;
     ...
     */
    function parseDiagram($content, &$linenum)
    {
	$this->content = '';
	$this->linkmap = array();
     ...
     */
    function initBoardAndDimensions()
    {
     ...
     */
    function initBordersAndGoban($diag)
    {
	$rows = explode("\n", $diag);
     ...
     */
    function parseSpecialMarkup($markup)
    {
	$res = array();
...
class GoDiagramPNG extends GoDiagram
{
     ...
     */
    function drawCoordinates($color)
    {
     ...
     */
    function drawIgnoredPoint($xpos, $ypos, $type)
    {
     ...
     */
    function getIntersectionType($position, $x, $y)
    {
	$type = '';
     ...
     */
    function drawIntersection($x, $y, $type, $color)
    {
	$r =& $this->dim->radius;
     ...
     */
    function drawStone ($x, $y, $Crim, $Cinside)
    {
       $d = $this->dim->radius*2;
     ...
     */
    function drawWildcard($x, $y, $type)
    {
       $r = $this->dim->radius;
     ...
     */
    function drawLetter($x, $y, $text, $color, $bold=false)
    {
     ...
     */
    function drawLine($x1,$y1, $x2,$y2, $fillcolor, $color, $arrow, $r=0)
    {
	if (!$r)
...
{
    var $heightdefined = false;	// board parameters
    var $widthdefined = false;
    var $sgfwidth;
    var $sgfheight;
    var $offsetX;	// SGF position offset
    var $offsetY;
    var $url;
...
function DemoEarReddeningMove()
{
    ...
    $diagramsrc = str_replace("<br />", "\n", $diagramsrc);
   // echo $diagramsrc;
/*'$$B The ear-reddening move
$$  ---------------------------------------
$$ | . . . . . . . . . X O O . . . . . . . |
$$ | . . . X . . . . . X O . O . O O X . . |
$$ | . . O O . X . . O X X O O . O X . . . |
